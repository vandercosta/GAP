...
public class MenuCliente extends javax.swing.JPanel implements ComandoJanelaInterna {

    ArrayList<Cliente> lista = new ArrayList<>();
    ClienteDAO dao = new ClienteDAO();
    MenuPrincipal menuPrincipal;
    TabelaCliente tabela = new TabelaCliente();
    ...
    private void btNovoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btNovoActionPerformed
        ...
        Processador.abrirJanela("novoCliente");
        ...
        if (jt.getSelectedRow() != -1) {
            ...
            if (JOptionPane.showConfirmDialog(null, "Deseja Excluir:", "AVISO", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE) == 0) {
                ...
                for (int i = 0; i < this.lista.size(); i++) {
                    if (this.lista.get(i).getIdCliente() == idCliente) {
                        clienteSelecionado = this.lista.get(i);
                        int linha = jt.getSelectedRow();
                        this.tabela.removeClientes(linha);
                        this.dao.excluir(clienteSelecionado);
                        break;
                    }
                }
            }
    ...
    public void buscarTodos() {
        ...
        this.tabela.setCliente(this.lista);
        jt.setModel(tabela);
    }
    ...
    public void editar() {
        ...
        if (jt.getSelectedRow() != -1) {
            ...
            for (int i = 0; i < this.lista.size(); i++) {
                if (this.lista.get(i).getIdCliente() == idCliente) {
                    clienteSelecionado = this.lista.get(i);
                    Processador.abrirEditar("editarCliente", clienteSelecionado);
                    break;
                }
            }
        } else {
